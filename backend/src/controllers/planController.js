...
const { planTemplates } = require('../templates/planTemplates')
...
async function updateProgress(req, res) {
  try {
    ...
    await prisma.dayTask.update({
      ...
      data: {
        status,
        completionRate: completionRate != null ? completionRate : dayTask.completionRate,
      },
    })
    ...
    if (status === 'completed') {
      ...
      if (plan) {
        const totalDays = plan.units.reduce((sum, u) => sum + u.days.length, 0)
        const completedDays = plan.units.reduce(
          (sum, u) => sum + u.days.filter((d) => d.status === 'completed').length,
          0
        )
        // 加上当前完成的这一天
    ...
    const allDaysCompleted = unitDays.every((d) => d.status === 'completed')
...
module.exports = { determineLevel, generatePlan, getPlan, getToday, updateProgress, adjustPlan }
